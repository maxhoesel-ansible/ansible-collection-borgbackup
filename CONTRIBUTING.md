...
Note that by contributing to this collection, you agree with the code of conduct you can find [here.](https://github.com/maxhoesel-ansible/ansible-collection-borgbackup/blob/main/CODE_OF_CONDUCT.md)
...
- `podman` for running tests (see [this section for details](#setting-up-podman))
...
There are currently no modules in this collection.
Here are some notes and pointers if you would like to write a module for a given use case:
...
- Use `doc_fragments` and `module_utils` python modules where applicable.
- Try to make modules idempotent where possible. Modules should always support check mode.
...
Why not just `molecule-docker`? Simple: This collection needs systemd inside its test containers to manages services and interface with the system, and modern versions of [systemd do not work inside docker containers anymore](https://github.com/ansible-community/molecule/discussions/3108).
...
- Pass a (partial) scenario name as a filter to limit execution (for example, `/tests/test-roles my-role` to limit molecule scenarios to the ACME role only).
...
provisioner:
  ...
  #inventory:
    #group_vars:
      #all:
        # Set cross-container inventory parameters here
scenario:
  test_sequence:
    - lint
    - destroy
...
- To update the python that is used to run the tests, bump the values in `tests/constants.sh`
